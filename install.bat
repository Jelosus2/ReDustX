...
rem Ensure pip is available in the embedded environment
if not exist "%PY_EMB_DIR%\Scripts\pip.exe" (
    echo.
...
rem Upgrade Python packaging tools before installing dependencies
"%PY_EXE%" -m pip install --upgrade --no-warn-script-location --disable-pip-version-check pip setuptools wheel
if errorlevel 1 (
    echo.
